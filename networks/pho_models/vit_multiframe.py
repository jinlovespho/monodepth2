...
class ViT_Multiframe(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, hybrid=False, 
                 pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., num_prev_frame=1, croco=True):
        super().__init__()
    ...
    def resize_pos_embed(self, h, w):
        ...
        for i, pos_emb in enumerate(self.pos_emb_lst):
            ...
            self.pos_emb_lst[i] = nn.Parameter(torch.cat([posemb_tok, posemb_grid], dim=1))     
